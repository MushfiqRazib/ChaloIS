...
Function GetSelectCount(conn, params, group)
  Dim rs, sql, sqlWhere, groupCondition
  ...
  If (params.Item("SQL_GroupBy") <> "NONE") Then
    ...
    If (group = NULLSTRING) Then
      groupCondition = params.Item("SQL_GroupBy") & " IS NULL"
    Else
      groupCondition = params.Item("SQL_GroupBy") & " = " & SQLEnc(group)
    End If
    
    '*** Add group condition to WHERE clause.
    sqlWhere = sqlWhere & IIf(sqlWhere <> "", " AND ", "") & "(" & groupCondition & ")"
  End If
...
Function GetSelectQuery(params, group, order_by, order_dir, row_first, row_last)
  Dim innerSQL, rownumSQL, limitSQL, sqlWhere, groupCondition
