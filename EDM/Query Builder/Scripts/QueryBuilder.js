...
function AddWhereClause() {
    ...
    if (oList) {    
        ...
        if (valuesInfo != "true")
        {
            ShowMessage('',valuesInfo,'i',null);
            return;
        }

        if (IsDuplicateListText("WhereList", value))
        {
            ShowMessage("Duplicate Entry","Duplicate Entry",'w',null);
            return;
        }        
...
{   
    ...
    {                 
        ...
        {
            Ext.Msg.alert("Duplicate Entry");
        }
    }else if(!expression)
    {
        Ext.Msg.alert("Expression not given");
    }else
    {
        Ext.Msg.alert("Alias not given");        
    }
}
...
function SelectGroupingField(elem)
{
    ...
    if (oSelect == null)
    {
        Ext.Msg.alert("Combobox 'GroupingType' niet gevonden!");    
        return false;
    }
...
function AddGroupByClause() {
    ...
    if (oList)
    {
        ...
        if (grpAlias == "")
        {
            ...
            Ext.Msg.alert("Alias value cannot be empty");
        }
        else if (IsValid(grpAlias.charAt(0), numRange))
        {
            ...
            Ext.Msg.alert('Alias can not start with numeric character.');
        }
        else
        {
...
function ExecuteGroupByOperation()
{
    ...
    {
        Ext.Msg.alert("Add group select clause");
        return;
    }
    
    if(sqlgroupby=="" && qbgbselectclause)
    {
        Ext.Msg.alert("Add group by field");
        return;
    }
