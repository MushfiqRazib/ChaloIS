...
public class EdmServicesForKit : System.Web.Services.WebService
{

    public EdmServicesForKit()
    {
    ...
    {
        DataTable dt = Security_Ext_DBHandler.GetUserSessionInfo(securityKey);
        if (dt.Rows.Count > 0)
        {
    ...
    public string SaveDocument(Byte[] fileChunk, int chunkNo, bool isNewFile, string relfileName)
    {
        ...
        string filePath = HIT.OB.STD.Wrapper.CommonFunctions.GetDocBasePath("DocBasePath") + relfileName;

        LogWriter.WriteLog(filePath);
    ...
    {
        string filePath = HIT.OB.STD.Wrapper.CommonFunctions.GetDocBasePath("DocBasePath") + relativeFilePath;
