...
namespace HIT.OB.STD.Wrapper.BLL
{
    public class Security_Ext_DBHandler
    {
        ...
        public static bool CheckValidity(string securityId)
        {
            string sql = string.Empty;
            bool retCode = false;
            try
            {
                ...
                {
                    connection.Open();
                    sql = @"select case when count(*)>0 then true else false end from securityinfo where securityId = @securityId;";
                    using (NpgsqlCommand cmd = new NpgsqlCommand(sql, connection))
                    {
                        cmd.Parameters.Add("securityId", securityId);
                        retCode = bool.Parse(cmd.ExecuteScalar().ToString());
        ...
        internal static string[] GetAuthenticationValues(string securityKey)
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    sql = @"select printserver_location,docSharePath, authentication_values from securityinfo where securityid = @securityKey;";
                    using (NpgsqlCommand cmd = new NpgsqlCommand(sql, connection))
                    {
            ...
            if (dt.Rows.Count > 0)
            {
                return new string[] { dt.Rows[0]["printserver_location"].ToString(),
                                        dt.Rows[0]["docSharePath"].ToString(),dt.Rows[0]["authentication_values"].ToString()};
            }
            return null;
        }
        ...
        public static string GetUserName(string securityKey)
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    sql = @"select userid from securityinfo where securityid = @securityKey";
        ...
        public static bool CheckSecurityIDExist(string sid) {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    sql = "select * from securityinfo where securityid = '" + sid + "'";
