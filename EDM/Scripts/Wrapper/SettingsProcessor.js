...
function SetSettingsToCore(repSettings, checkResult)
{
    ...
    OBSettings.SQL_SEARCH_COLUMN_GRID_INDEX = (repSettings.sql_search_column_grid_index != "") ? repSettings.sql_search_column_grid_index : "";
    OBSettings.SQL_LAST_QUICK_SEARCH_STRING = (repSettings.sql_last_quick_search_string != "") ? repSettings.sql_last_quick_search_string : "";
    OBSettings.SQL_SEARCH_COLUMN = (repSettings.SQL_SEARCH_COLUMN != "") ? repSettings.SQL_SEARCH_COLUMN : "";
...
{    
    ...
    {             
        if(Get_Cookie("REPORT_CODE"))
        {
            ...
            var sqlSearchColumnGridIndex = Get_Cookie("SQL_SEARCH_COLUMN_GRID_INDEX");
            var sqlLastSearchString = Get_Cookie("SQL_LAST_QUICK_SEARCH_STRING");
            var sqlSearchColumnWithType = Get_Cookie("SQL_SEARCH_COLUMN");
            ...
            settings['sql_search_column_grid_index'] = sqlSearchColumnGridIndex;
            settings['sql_last_quick_search_string'] = sqlLastSearchString;
            settings['SQL_SEARCH_COLUMN'] = sqlSearchColumnWithType;
            return settings;
        }
    }catch(e)
    {
...
function SaveUserSettingsInCookie()
{
    ...
    {    
        ...
        Set_Cookie("SQL_SEARCH_COLUMN_GRID_INDEX", OBSettings.SQL_SEARCH_COLUMN_GRID_INDEX,1);
        Set_Cookie("SQL_LAST_QUICK_SEARCH_STRING", OBSettings.SQL_LAST_QUICK_SEARCH_STRING,1); 
        Set_Cookie("SQL_SEARCH_COLUMN", OBSettings.SQL_SEARCH_COLUMN,1);        
    }
}
...
function GetCurrentVisibleFieldNamesGGrid()
{
	var selectedFieldsName = "";
    try {
        
        var currentVisibleFields = new Array();
        var currentGrid = OBSettings.groupedGrid; //GetCurrentActiveGrid();    
...
function GetCurrentVisibleFieldNamesNGrid()
{
	var selectedFieldsName = "";
    try {
        
        var currentVisibleFields = new Array();
        var currentGrid = OBSettings.grid; //GetCurrentActiveGrid();    
    ...
    try {
        ...
        if (currentGrid) {
            ...
            for (var i = 0; i < currentVisibleFields.length; i++) {
                if (currentVisibleFields[i] != undefined) {
                    visibleFieldsWithWidth = currentVisibleFields[i].split('*');
                    var fieldName = currentVisibleFields[i].substring(0, currentVisibleFields[i].lastIndexOf('*'));
                    if (IsExistInSqlSelect(fieldName.trim())) {                        
                        selectedFieldsName += fieldName.trim() + '*' + visibleFieldsWithWidth[visibleFieldsWithWidth.length - 1] + ";";
                    }
                }
            }           
        }
...
function GetCurrentVisibleFieldNamesWithWidthNrGrid()
{
...
function isExistInGruop(gcol)
{
   
     var colls = OBSettings.groupedGrid.colModel.config;
     for (var i = 0; i < colls.length; i++) 
     {
        if (gcol.dataIndex == colls[i].dataIndex )
            return true;
        
     }
     return false;
}
function GetCurrentHiddenFieldGNames() {
    ...
    try {
        ...
        var currentVisibleFields = new Array();
        //var currentGrid = OBSettings.groupedGrid; //GetCurrentActiveGrid();    
        var currentGrid = OBSettings.gridStore; //GetCurrentActiveGrid();    
        
        if (currentGrid) {
            //var colls = currentGrid.colModel.config;
            
             for (k = 0; k < currentGrid.getCount(); k++)
                {
                   
                    rec = currentGrid.getAt(k);
                                    
                //if (colls[i] && colls[i].hidden === false && isDataField(colls[i].header) && !isExistInGruop(colls[i]) ) {
                    if (rec.data.expression != "" )
                        selectedFieldsName  += rec.data.expression + '#' + rec.data.width +";";
                    else
                        selectedFieldsName  += rec.data.fieldName + '#' + rec.data.width +";";
                }
...
function GetCurrentHiddenFieldNamesNLGrid() {
    ...
    try {
        ...
        if (currentGrid) {
            var colls = currentGrid.colModel.config;
            for (var i = 0; i < colls.length; i++) {
                if (colls[i] && colls[i].hidden === true && isDataField(colls[i].header)) {
                    currentVisibleFields[i] = colls[i].dataIndex + '#' + parseInt(colls[i].width);
                }
            }
...
function GetCurrentHiddenFieldNames() {
...
function ClearUserSettingsFromCookie()
{
    ...
    /* Start: To delete the quick search items from cookies */
    Delete_Cookie("SQL_SEARCH_COLUMN_GRID_INDEX");
    Delete_Cookie("SQL_LAST_QUICK_SEARCH_STRING"); 
    Delete_Cookie("SQL_SEARCH_COLUMN"); 
    /* End: To delete the quick search items from cookies */     
