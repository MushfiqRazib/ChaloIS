...
var OBCore = function(showTabPanel) {
    ...
    this.SQL_FROM = "";
    this.SQL_WHERE = "";
    this.SQL_WHERE_CMPVALUE = "";
    this.SQL_ORDER_BY = "";
    this.SQL_ORDER_DIR = "ASC";
    this.SQL_SEARCH_COLUMN = "";
    this.SQL_SEARCH_COLUMN_GRID_INDEX = "";
    this.SQL_LAST_QUICK_SEARCH_STRING = "";
    ...
    this.FIXED_FIELD_SIZE_LIST = { '#': 30, ' ': 25, '': 25, 'Nr': 55, 'ADD': 25, 'EDIT': 30, 'DELETE': 25, 'ZOOM': 25, 'VIEW': 25, 'BASKET': 25, 'BASKETWITHPARTLIST': 25, 'THEME_COLOR': 23, 'GROUP2FILTER': 25, 'EXPANDCOLLAPSE': 25, 'MULTISELECT': 25 };
    ...
    this.SQL_FIELD_TYPE = {};
    this.TOOLTIPS = { 'ADD': 'Add', 'EDIT': 'Edit', 'DELETE': 'Delete', 'ZOOM': 'Zoom', 'VIEW': 'View', 'GROUP2FILTER': 'Set current group as filter', 'BASKET': 'Add record to basket', 'BASKETWITHPARTLIST': 'Add record to basket including all possible subparts and their subparts', 'collapse': 'Expand' };
...
OBCore.prototype =
{
    CreateNormalGrid: function() {
        //debugger;
        try {
            if (OBSettings.groupByEvent) {
                  var sql = OBSettings.SQL_SELECT; 
                  if(sql.search("count")!=-1){                   
                   sql = OBSettings.SQL_SELECT.toLowerCase();
                   sql = sql.substring(0,sql.indexOf('count')-1);
                   this.SQL_SELECT = sql;
                   //alert(this.SQL_SELECT);
                  }
                
                LoadDatabaseSettings();
                OBSettings.groupByEvent = false;
            }
            ...
            this.grid = new Ext.grid.GridPanel({
                ...
                listeners: {
                    ...
                    afterrender: function() {
                        ...
                        //*** Activate deep link tab
                        tabPanel.activate(tabPanel.items.items[deepLinkTab]);
                        if (deepLinkTab != 1) {
                            ClearDeeplinkInfo();
                        }

                        /* the rest of the part of this after render is needed to manage quicksearch */
                        if (((OBSettings.SQL_LAST_QUICK_SEARCH_STRING) && (OBSettings.SQL_LAST_QUICK_SEARCH_STRING != "") && (OBSettings.SQL_LAST_QUICK_SEARCH_STRING != "undefined")) && OBSettings.SQL_WHERE != "") {
                            var searchTxtInput = Ext.get('txtSearch');
                            //searchTxtInput.dom.value = OBSettings.SQL_LAST_QUICK_SEARCH_STRING;
                            searchTxtInput.dom.style.color = 'black';
                            searchTxtInput.dom.style.fontStyle = 'normal';
                        }
                        else {
                            var searchTxtInput = Ext.get('txtSearch');
                            searchTxtInput.dom.value = "Enter search string";
                            searchTxtInput.dom.style.color = 'gray';
                            searchTxtInput.dom.style.fontStyle = 'italic';
                        }

                        if (OBSettings.SQL_SEARCH_COLUMN_GRID_INDEX && OBSettings.SQL_SEARCH_COLUMN_GRID_INDEX != "undefined") {
                            this.getView().getHeaderCell(OBSettings.SQL_SEARCH_COLUMN_GRID_INDEX).children[0].style.backgroundColor = '#dfe8f6';
                            this.oldSelectedColumn = OBSettings.SQL_SEARCH_COLUMN_GRID_INDEX;
                        }
                        /*End of code for quick search managing when render */
                    },
                    mouseover: function(ev) {
                        ev.stopPropagation();
                    },
                    headerClick: function(g, index, e) {
                        /* this part of this is needed to manage quicksearch */
                       
                        /* inside this if statement we return if its not a database column to search, no selection for this column */
                        if (index == 0 || (g.view.cm.config[index].header === "")) {
                            return false;
                        }
                        if (this.oldSelectedColumn) {
                            g.getView().getHeaderCell(this.oldSelectedColumn).children[0].style.backgroundColor = '';
                        }
                        this.oldSelectedColumn = index;

                        OBSettings.SQL_SEARCH_COLUMN = g.getView().cm.getDataIndex(index);

                        OBSettings.SQL_SEARCH_COLUMN_GRID_INDEX = index;
                        g.getView().getHeaderCell(index).children[0].style.backgroundColor = '#dfe8f6';

                        if (Ext.getCmp('mainSettingsPanel') && OBSettings.SQL_GROUP_BY == 'NONE') {
                            for (var i = 0; i < Ext.getCmp('nrgridid').getView().ds.data.items.length; i++) {
                                var item = Ext.getCmp('nrgridid').store.data.items[i];
                                if (item.data.caption == g.view.cm.config[index].header) {
                                    Ext.getCmp('nrgridid').selModel.select(i, 0);
                                    Ext.getCmp('ntxfcaptionid').setValue(item.data.caption);
                                    Ext.getCmp('ntxfwidthid').setValue(item.data.width);
                                    Ext.getCmp('lblLexp').setValue(item.data.fieldName);
                                }
                            }
                        }
                        ...
                        return false;
                        /* End of code for quick search managing when click on grid column header */
                    }
                }
            ...
            this.grid.on("columnresize", function(i, w) {
                if (Ext.getCmp('nrgridid') && Ext.getCmp('nrgridid').selModel.selection) {
                    if (Ext.getCmp('NormalGrid').colModel.getColumnAt(i).header == Ext.getCmp('nrgridid').selModel.selection.record.data.caption) {
                        Ext.getCmp('ntxfwidthid').setValue(w);
                        Ext.getCmp('nrgridid').selModel.selection.record.data.width = w.toString();
                    }
                    else {
                        Ext.getCmp('nrgridid').store.data.each(function(item) {
                            if (item.data.caption == Ext.getCmp('NormalGrid').colModel.getColumnAt(i).header) {
                                item.data.width = w.toString();
                                return;
                            }
                        });
                    }
            ...
            this.grid.width = "100%";
            this.grid.title = '';
            this.grid.viewConfig = { autoFill: false, forceFit: false };
            ...
            if (this.ACTIVE_GRID == 'MAIN_GRID')  //For Main grid
            {
                ...
                Ext.get('gridContainer'); //.removeAll();
            ...
            /* 
            *This function will reload the Component panel i.e. querybuilder, basktet etc.
            *If it was opened in the previous page.
            **/
            ReLoadComponentPanels();
        } catch (e) {

            ShowMessage('Error', "Json Error, Failed to create report", 'e', null);
    ...
    CreateGroupByGrid: function() {
        try {
            ...
            this.groupedGrid.on("columnresize", function(i, w) {
                if (Ext.getCmp('nrgridid') && Ext.getCmp('nrgridid').selModel.selection) {
                    if (Ext.getCmp('GroupGrid').colModel.getColumnAt(i).header == Ext.getCmp('nrgridid').selModel.selection.record.data.caption) {
                        Ext.getCmp('ntxfwidthid').setValue(w);
                    }
                }
            });
            ...
            ShowMessage('Error', "Json Error, Failed to create report", 'e', null);
            //Ext.Msg.alert("Json Error, Failed to create report");
            this.APP_SETTINGS_SAVEABLE = false;
        }
    },
    ...
    GetNormalGridData: function() {
        ...
        //This unescape and consequent escape is required to handle 
        //encoded where recieved from Deepling url.
        var sqlWhere = unescape(this.SQL_WHERE);
        sqlWhere = escape(this.SQL_WHERE);
        ...
        if (deepLinkWhere) {
            OBSettings.SQL_WHERE = deepLinkWhere;
            sqlWhere = escape(deepLinkWhere);
        }
    ...
    ResetRowNumberColumnSize: function() {
        if (this.ACTIVE_GRID == "MAIN_GRID") {
            this.FIXED_FIELD_SIZE_LIST['#'] = (this.START_ROW + 100).toString().length * 10;
        } else {
            this.FIXED_FIELD_SIZE_LIST['#'] = (this.DETAIL_GRID_START_ROW + 100).toString().length * 10;
        }
    ...
    GetNonGridHeight: function() {
        ...
        height += 64; //Added 64 to make the grids lower items visible.
        return height;
    },
    ...
    ResizeGrid: function() {
        ...
        if (Ext.get('containerDiv').getHeight() > 0) {

            var grid1 = Ext.get('NormalGrid').select('.x-grid3-scroller').elements[0];
            if (Ext.get('queryBuilderPanel') != null) {
                var curComp = Ext.getCmp('queryBuilderPanel');
                //set height
                ...
                if (Ext.isDefined(curComp)) {
                    ...
                    grid1.style.height = height - 170 + "px";


                }
            }

            else if (Ext.get('mainReportPanel') != null) {
                var curComp = Ext.getCmp('mainReportPanel');
                //set height
            ...
            else if (Ext.get('mainSettingsPanel') != null) {
                var curComp = Ext.getCmp('mainSettingsPanel');
                //set height
            ...
            else if (Ext.get('basketPanel') != null) {
                var curComp = Ext.getCmp('basketPanel');
                //set height
                  var hh =  Ext.get('containerDiv').getHeight();
                if (Ext.isDefined(curComp)) {
    ...
    IsColumnVisible: function(field) {
        ...
        if (this.QB_GB_SELECT_CLAUSE == field) {
            return true;
        }
        for (var k = 0; k < this.NON_DRAGGABLE_FIELD_LIST.length; k++) {
    ...
    QuickSearchOnUserData: function() {
        ...
        var userSearchString = trim(Ext.get('txtSearch').getValue());
        //userSearchString = userSearchString.replace("\\","\\\\\\\\");
        if (userSearchString == "Enter search string" || userSearchString == "") {
            OBSettings.SQL_LAST_QUICK_SEARCH_STRING = "";
            ShowMessage('Quick Search Message', 'Enter search string.', 'w', null);
            //Ext.Msg.alert('Quick Search Message', 'Enter search string.');
            return;
        }


        //var operator = Ext.get('quickSearchOperator').dom.value;
        var searchField = this.GetQuickSearchableField();
        if (searchField == "" || searchField == "undefined") {
            ShowMessage('Quick Search Message', 'Select a column first.', 'w', null);
            //Ext.Msg.alert('Quick Search Message', 'Select a column first.');
            var searchTxtInput = Ext.get('txtSearch');
            searchTxtInput.dom.value = userSearchString;
            searchTxtInput.dom.style.color = 'black';
            searchTxtInput.dom.style.fontStyle = 'normal';
            return;
        }

        this.SQL_LAST_QUICK_SEARCH_STRING = userSearchString;
        userSearchString = userSearchString.toUpperCase();
        var fldtpe = this.SQL_FIELD_TYPE[this.GetOriginalFieldName(searchField)];

        if (userSearchString && userSearchString != "Enter search srting") {
            userSearchString = "LIKE '%" + userSearchString + "%'";
            if (fldtpe != "STRING") {
                if (fldtpe.indexOf("DATE") != -1 || fldtpe.indexOf("TIMESTAMP") != -1) {
                    userSearchString = "to_char(" + searchField + ", 'DD-Mon-YYYY') " + userSearchString;
                }
                else if (fldtpe.indexOf("NUMERIC") != -1) {
                    userSearchString = "to_char(" + searchField + ", '99999999D') " + userSearchString;
                }
            }
            else {
                userSearchString = "upper(" + searchField + ") " + userSearchString;
            }
            //alert(userSearchString);
            ...
            if (isFilterStringValid == "true") {
                ...
                ShowMessage('Quick Search Message', isFilterStringValid, 'w', null);
                //Ext.Msg.alert('Quick Search Message', isFilterStringValid);
                return;
            }
    ...
    GetQuickSearchableField: function() {
        var field = "";
        if (this.SQL_SEARCH_COLUMN) {
            field = this.SQL_SEARCH_COLUMN;
        }
        ...
        //            fieldName = OBSettings.GetDelimittedKeyValuePair('$').split('$')[0];

        //        }

        // truncate the the alias name with as if exists
        //        if (field.match(/ AS /i)) {
        //            field = field.split(/ AS /i)[0];
        //        }
        return field;
    },
    ...
    UpdateSortInfoInNavigation: function() {
        ...
        Ext.fly('lblSortedFieldName').update(trim(fieldName));
    ...
    getOperator: function(fieldName) {
        fieldName = this.GetOriginalFieldName(fieldName);
        var fldType = this.SQL_FIELD_TYPE[fieldName];
        var optList;
